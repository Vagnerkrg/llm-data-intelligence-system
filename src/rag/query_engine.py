...
class RAGQueryEngine:
    ...
    def __init__(self, vector_index_path="models/vector_index.pkl"):
    ...
    def retrieve(self, question, top_k=3, min_score=0.40):
        ...
        results = self.vector_index.search(embedding, top_k=top_k, source=source)
        ...
        if results:
            ...
            if best_score < min_score:
                results = self.vector_index.search(embedding, top_k=top_k)
...
"""
    ...
    def query(self, question, top_k=3):
        ...
        results, route, fallback_used = self.retrieve(question, top_k)

        context = self.build_context(results)

        prompt = self.prompt_template.build(context, question)

        answer = self.llm.generate(prompt)

        metrics = self.metrics.evaluate(question, route, results, fallback_used)

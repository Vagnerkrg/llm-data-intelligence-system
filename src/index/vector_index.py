...
class VectorIndex:
    ...
    def __init__(self, index_path="models/vector_index.pkl"):
    ...
    def search(self, query_embedding, top_k=3, source=None):
        """
        ...
        indexes_pool = range(len(self.documents))

        if source:
            indexes_pool = [
                index
                for index in indexes_pool
                if self.metadata[index].get("source") == source
            ]

        if not indexes_pool:
            return []

        filtered_vectors = vectors[list(indexes_pool)]

        similarities = (
            filtered_vectors
            @ query
            / (np.linalg.norm(filtered_vectors, axis=1) * np.linalg.norm(query))
        )

        sorted_positions = np.argsort(similarities)[::-1][:top_k]

        results = []

        for position in sorted_positions:
            original_index = list(indexes_pool)[position]

...
class DocumentBuilder:
    ...
    def build_from_dataframe(
        self, dataframe: pd.DataFrame, dataset_name: str, limit: int = None
    ) -> List[Dict]:
        """
        ...
        for _, row in dataframe.iterrows():
            document = self._build_document(row, dataset_name)
    ...
    def build_from_datasets(
        self, datasets: Dict[str, pd.DataFrame], limit: int = 50
    ) -> List[Dict]:
        """
    ...
    def _build_document(self, row, dataset_name):
        ...
        return self._generic_document(row, dataset_name)

    def _create_document(self, text, source, document_type):

        return {"text": text, "metadata": {"source": source, "type": document_type}}

    def _product_document(self, row):
...
"""
        ...
        return self._create_document(text, "products", "product")

    def _review_document(self, row):
...
"""
        ...
        return self._create_document(text, "reviews", "review")

    def _order_document(self, row):
...
"""
        ...
        return self._create_document(text, "orders", "order")

    def _customer_document(self, row):
...
"""
        ...
        return self._create_document(text, "customers", "customer")

    def _generic_document(self, row, dataset_name):

        information = "\n".join([f"{column}: {value}" for column, value in row.items()])

        return self._create_document(information, dataset_name, "generic")

...
class EvaluationRunner:
    ...
    def __init__(self, questions_path=None, output_path="evaluation_report.json"):

        self.questions_path = Path(
            questions_path if questions_path else "tests/evaluation/rag_questions.json"
        )
    ...
    def run(self):
        ...
        for item in questions:
            ...
            retrieval_scores.append(metrics["best_score"])

            if metrics["fallback_used"]:
                fallback_count += 1

            if expected not in domain_stats:
                domain_stats[expected] = {"questions": 0, "correct": 0, "scores": []}
            ...
            domain_stats[expected]["scores"].append(metrics["best_score"])

            answer_evaluation = self.answer_evaluator.evaluate(
                item["question"], response["answer"], response["sources"]
            )

            answer_results.append(answer_evaluation)
        ...
        for domain, stats in domain_stats.items():
            formatted_domain_metrics[domain] = {
                ...
                "accuracy": round(stats["correct"] / stats["questions"], 4),
                "average_score": round(sum(stats["scores"]) / len(stats["scores"]), 4),
            }

        answer_quality_report = self.benchmark_report.generate(answer_results)

        report = {
            "total_questions": len(questions),
            "correct_predictions": correct,
            "routing_accuracy": round(correct / len(questions), 4),
            "average_retrieval_score": round(
                sum(retrieval_scores) / len(retrieval_scores), 4
            ),
            "fallback_rate": round(fallback_count / len(questions), 4),
...
if __name__ == "__main__":
    ...
    runner = EvaluationRunner(questions_path=questions_path)

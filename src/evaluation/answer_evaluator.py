...
class AnswerEvaluator:
    ...
    def evaluate(self, question: str, answer: str, documents: List[Dict]) -> Dict:
        """
        Generates answer quality metrics.
        """

        context = " ".join([item.get("document", "") for item in documents]).lower()
        ...
        relevance_score = self._calculate_overlap(question_words, context_words)

        faithfulness_score = self._calculate_overlap(answer_words, context_words)

        completeness_score = min(len(answer_words) / 100, 1.0)

        quality_score = round(
            (relevance_score + faithfulness_score + completeness_score) / 3, 4
        )
        ...
        return {
            ...
            "context_relevance": round(relevance_score, 4),
    ...
    def _extract_words(self, text: str) -> set:
        ...
        return set(word.strip() for word in words if len(word.strip()) > 3)

    def _calculate_overlap(self, first_set: set, second_set: set) -> float:
        """

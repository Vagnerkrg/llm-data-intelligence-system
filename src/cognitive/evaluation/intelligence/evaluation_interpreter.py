...
class EvaluationInterpreter:
    ...
    def interpret(self, result: EvaluationResult) -> EvaluationInterpretation:

        areas = []

        for signal in result.signals:
            if signal.impact == "high":
                areas.append(signal.signal_type)

        requires_improvement = result.score < 0.7 or len(areas) > 0
        ...
        return EvaluationInterpretation(
            severity=severity, requires_improvement=requires_improvement, areas=areas
        )

...
class OptimizationSignal:
    ...
    def __post_init__(self) -> None:
        if not self.signal_type.strip():
            raise ValueError(
                "Optimization signal type cannot be empty."
            )

        if not self.target.strip():
            raise ValueError(
                "Optimization signal target cannot be empty."
            )

        if not self.direction.strip():
            raise ValueError(
                "Optimization signal direction cannot be empty."
            )

        if not 0.0 <= self.strength <= 1.0:
            raise ValueError(
                "Optimization signal strength must be between "
                "0.0 and 1.0."
            )

        if not 0.0 <= self.confidence <= 1.0:
            raise ValueError(
                "Optimization signal confidence must be between "
                "0.0 and 1.0."
            )

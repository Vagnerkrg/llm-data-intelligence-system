...
class AgentIntelligence:
    ...
    def __init__(
        ...
        self.orchestrator = orchestrator if orchestrator else AgentOrchestrator()

    def process(self, question: str) -> IntelligenceResult:
        """
        ...
        orchestration_result = self.orchestrator.execute(question)

        return IntelligenceResult(
            answer=(
                orchestration_result.result
                if hasattr(orchestration_result, "result")
                else str(orchestration_result)
            ),
            confidence=(reasoning_result.confidence),
            reasoning=(reasoning_result.reasoning),
            metadata={"component": "agent_intelligence"},
        )

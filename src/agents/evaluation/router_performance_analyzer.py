...
class RouterPerformanceAnalyzer:
    ...
    def __init__(self, history: RoutingHistory):
    ...
    def success_rate_by_tool(self) -> Dict[str, float]:
        """
        ...
        return {tool: (data["success"] / data["total"]) for tool, data in stats.items()}

    def average_confidence_by_tool(self) -> Dict[str, float]:
        """
        ...
        for record in self.history.all():
            ...
            confidence[tool].append(record.confidence)

        return {
            tool: (sum(values) / len(values)) for tool, values in confidence.items()
        }

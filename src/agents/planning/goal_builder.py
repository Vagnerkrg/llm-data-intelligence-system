...
class GoalBuilder:
    ...
    def build(self, reasoning_result: ReasoningResult) -> Goal:
        """
        ...
        objective = (
            ...
            or metadata.get("user_request")
            or getattr(reasoning_result, "goal", None)
            or getattr(reasoning_result, "conclusion", None)
            or "Solve user request"
        )

        intent = getattr(reasoning_result, "intent", "general")

        required_capabilities = getattr(reasoning_result, "required_capabilities", [])
        ...
        return Goal(
            ...
            metadata={
                ...
                "source": (metadata.get("source", "goal_builder")),
            },
        )

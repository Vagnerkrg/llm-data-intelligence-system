...
class AdaptiveReplanningPolicy:
    ...
    def evaluate(
        ...
        if feedback.get("requires_adjustment"):
            return ReplanningDecision(
                should_replan=True,
                reason="feedback_requires_adjustment",
                strategy="adaptation",
                issues=["feedback_adjustment_required"],
            )

        if execution_state.get("blocked"):
            return ReplanningDecision(
                should_replan=True,
        ...
        return ReplanningDecision(
            should_replan=False, reason="execution_progress_normal", strategy="continue"
        )

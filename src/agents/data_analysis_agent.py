...
class DataAnalysisAgent:
    ...
    def __init__(
        self, intelligence_service=None, analytics_engine=None, repository=None
    ):
        ...
        if intelligence_service:
            ...
            if analytics_engine:
                ...
                analytics = AnalyticsEngine(repository=repository)

                self.intelligence = DataIntelligenceService(analytics_engine=analytics)
    ...
    def run(self, question: str) -> Dict:
        ...
        self.logger.info(f"[ANALYSIS_AGENT] Question received: {question}")

        text = question.lower()

        if "quantos produtos" in text:
            result = self.intelligence.count_dataset_rows("products")
        ...
        if "quantos clientes" in text:
            result = self.intelligence.count_dataset_rows("customers")
        ...
        if "colunas" in text:
            result = self.intelligence.get_dataset_columns("products")
        ...
        if "categoria" in text and (
            "mais" in text or "maior" in text or "possui" in text
        ):
            result = self.intelligence.get_value_counts(
                "products", "product_category_name", limit=5
            )

...
class MemoryLifecycleResult:
    ...
    def __init__(
        self, memory_id: str = "", consolidated: bool = False, decayed: bool = False
    ):
...
class MemoryLifecycleManager:
    ...
    def __init__(self, consolidator: MemoryConsolidator, decay: MemoryDecay):

        self.consolidator = consolidator

        self.decay = decay

    def process(self, memory: MemoryEntry) -> MemoryLifecycleResult:
        """
        ...
        consolidation = self.consolidator.consolidate(memory)

        decay_result = self.decay.apply(memory)

        if isinstance(decay_result, dict):
            decayed = decay_result.get("decayed", True)

        else:
            decayed = getattr(decay_result, "decayed", False)

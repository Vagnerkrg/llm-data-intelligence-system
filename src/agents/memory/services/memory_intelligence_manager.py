...
from src.agents.memory.services.memory_lifecycle_manager import MemoryLifecycleManager
...
from src.agents.memory.intelligence.memory_relevance_scorer import MemoryRelevanceScorer
...
class MemoryIntelligenceManager:
    ...
    def __init__(
        ...
        self.relevance_scorer = (
            relevance_scorer if relevance_scorer else MemoryRelevanceScorer()
        )

        self.lifecycle_manager = (
            lifecycle_manager if lifecycle_manager else self._create_lifecycle_manager()
        )

    def _create_lifecycle_manager(self) -> MemoryLifecycleManager:
        """
        ...
        consolidator = MemoryConsolidator(relevance_analyzer)

        decay = MemoryDecay()

        return MemoryLifecycleManager(consolidator=consolidator, decay=decay)

    def analyze(self, memory: MemoryEntry) -> RelevanceResult:
        """
        ...
        if not isinstance(memory, MemoryEntry):
            result = RelevanceResult(
                memory_id="", score=0.0, explanation="Invalid memory."
            )
        ...
        relevance = self.relevance_scorer.evaluate(memory)

        lifecycle = self.lifecycle_manager.process(memory)

        relevance.lifecycle = lifecycle

        relevance.relevance_score = relevance.score

...
class StepExecutor:
    ...
    def _mark_running(self, step):

        if hasattr(step, "mark_running"):
            step.mark_running()

    def _complete(self, step, result):
    ...
    def _fail(self, step, error):
        ...
        if hasattr(step, "fail"):
            step.fail(error)

            return

    def execute(self, step, question: str):
        """

...
class AgentController:
    ...
    def __init__(
        ...
        self.router = router if router else AgentRouter(self.tool_registry)

        self.execution_executor = (
            execution_executor if execution_executor else ToolExecutor()
        )

        self.orchestrator = (
            orchestrator
            if orchestrator
            else ToolOrchestrator(self.tool_registry, self.execution_executor)
        )

        self.planner = planner if planner else ExecutionPlanner()
    ...
    def create_plan(self, question: str) -> ExecutionPlan:
        ...
        return self.planner.create_plan(question)
    ...
    def handle_request(self, question: str):
        ...
        if self.runtime:
            return self.runtime.execute(question)
    ...
    def run(self, question: str) -> Dict:
        ...
        tools = [routing_result.tool]

        orchestration_result = self.orchestrator.execute(tools, question)

        results = orchestration_result.get("results", [])

        if not results:
            return {"status": "error", "message": ("No execution result.")}
        ...
        return {
            ...
            "tools_executed": (orchestration_result.get("tools_executed", [])),
            ...
            "result": (
                results[0]["data"]
                if len(results) == 1 and "data" in results[0]
                else results
            ),
            # Novo contrato V1.10
            "results": results,

from typing import Any

from ..domain.evaluation_context import EvaluationContext
from ..domain.evaluation_metric import EvaluationMetric


class MemoryEffectivenessMetric:
    """
    Evaluates the effectiveness of memory usage.

    The metric considers:

    - memory usage
    - relevance score
    - retrieved context quality
    - memory contribution
    """

    metric_name = "memory_effectiveness"
    metric_category = "memory"

    def evaluate(
        self,
        context: EvaluationContext,
    ) -> EvaluationMetric:
        """
        Evaluate memory effectiveness from an evaluation context.
        """

        information = context.memory_information

        if not information:
            return EvaluationMetric(
                name=self.metric_name,
                category=self.metric_category,
                score=0.0,
                description="Memory information is unavailable.",
            )

        if not isinstance(information, dict):
            return EvaluationMetric(
                name=self.metric_name,
                category=self.metric_category,
                score=0.0,
                description="Invalid memory information.",
            )

        factors = [
            self._normalize_value(information.get("memory_usage")),
            self._normalize_value(information.get("relevance_score")),
            self._normalize_value(information.get("retrieved_context_quality")),
            self._normalize_value(information.get("memory_contribution")),
        ]

        available = [factor for factor in factors if factor is not None]

        score = sum(available) / len(available) if available else 0.0

        return EvaluationMetric(
            name=self.metric_name,
            category=self.metric_category,
            score=score,
            metadata={
                "factors_evaluated": len(available),
                "factors_available": len(factors),
            },
            description="Memory effectiveness evaluated.",
        )

    @staticmethod
    def _normalize_value(value: Any) -> float | None:
        """
        Normalize a metric component to the [0.0, 1.0] range.
        """

        if value is None:
            return None

        if isinstance(value, bool):
            return float(value)

        if isinstance(value, (int, float)):
            return max(0.0, min(1.0, float(value)))

        return None

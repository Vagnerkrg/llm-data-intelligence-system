...
def test_weighted_strategy_single_evaluation():
    ...
    evaluation = CapabilityEvaluation(capability="reasoning", score=0.8, weight=1.0)

    result = strategy.evaluate([evaluation])
...
def test_weighted_strategy_multiple_evaluations():
    ...
    evaluations = [
        CapabilityEvaluation(capability="reasoning", score=0.8, weight=2.0),
        CapabilityEvaluation(capability="memory", score=0.6, weight=1.0),
    ]

    result = strategy.evaluate(evaluations)

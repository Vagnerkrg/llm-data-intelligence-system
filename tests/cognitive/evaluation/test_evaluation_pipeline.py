from src.cognitive.evaluation.evaluation_pipeline import EvaluationPipeline
...
class DummyEvaluator:
    def evaluate(self, context):
        ...
        return EvaluationResult(score=0.9, passed=True, evaluator="dummy")
...
def create_pipeline():
    ...
    registry.register("dummy", DummyEvaluator())

    engine = EvaluationEngine(registry)

    return EvaluationPipeline(engine)
...
def test_pipeline_run_single():
    ...
    result = pipeline.run_single("dummy", context)
...
def test_pipeline_invalid_evaluator():
    ...
    try:
        pipeline.run_single("unknown", context)

...
class TestAnswerGenerator:
    ...
    def test_generate_count_rows_response(self):
        ...
        result = {"operation": "count_rows", "dataset": "products", "result": 32951}

        response = self.generator.generate(result)

        assert response == "O dataset products possui 32951 registros."
    ...
    def test_generate_columns_response(self):
        ...
        result = {
            "operation": "columns",
            "dataset": "products",
            "result": ["product_id", "product_category_name"],
        }

        response = self.generator.generate(result)

        assert (
            response == "As colunas disponíveis são: product_id, product_category_name"
        )
    ...
    def test_generate_empty_result_response(self):
        ...
        assert response == "Não encontrei informações suficientes para responder."

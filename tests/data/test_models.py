...
from src.data.models import DataSource, DatasetInfo, LoadResult


def test_datasource_creation():

    source = DataSource(name="products", path="data/raw/products.csv", format="csv")
...
def test_dataset_info_creation():
    ...
    info = DatasetInfo(name="products", rows=100, columns=5)
...
def test_load_result_creation():
    ...
    info = DatasetInfo(name="test", rows=2, columns=2)

    result = LoadResult(name="test", dataframe=dataframe, info=info)

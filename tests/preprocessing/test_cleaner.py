...
def test_clean_text_columns():
    ...
    dataframe = pd.DataFrame({"name": [" Product A ", " Product B "]})
...
def test_remove_empty_columns():
    ...
    dataframe = pd.DataFrame({"id": [1, 2], "empty": [None, None]})
...
def test_remove_duplicates():
    ...
    dataframe = pd.DataFrame({"id": [1, 1, 2]})
...
def test_clean_pipeline():
    ...
    dataframe = pd.DataFrame({"name": [" A ", " A "], "empty": [None, None]})

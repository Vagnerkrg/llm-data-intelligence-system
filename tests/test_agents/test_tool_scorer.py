...
def test_tool_scorer_matches_capability():
    ...
    tools = [
        ToolMetadata(
            ...
            capabilities=["statistics", "aggregation"],
        )
    ]

    result = scorer.score("execute statistics analysis", tools)
...
def test_tool_scorer_returns_empty_when_no_match():
    ...
    result = scorer.score("explain a story", tools)
...
def test_tool_scorer_orders_by_score():
    ...
    tools = [
        ToolMetadata(
            name="basic", description="Basic tool", capabilities=["statistics"]
        ),
        ToolMetadata(
            name="advanced",
    ...
    result = scorer.score("statistics aggregation", tools)

...
def test_tool_orchestrator_executes_tool():
    ...
    result = orchestrator.execute(["mock_tool"], "test question")

    assert result["status"] == "success"

    assert result["tools_executed"] == 1

    assert result["results"][0]["tool"] == "mock_tool"
...
def test_tool_orchestrator_handles_missing_tool():
    ...
    result = orchestrator.execute(["unknown_tool"], "test")

...
def test_goal_builder_creates_goal_from_reasoning():
    ...
    goal = builder.build(reasoning)

    assert goal is not None

    assert goal.objective == ("Analyze sales patterns")

    assert goal.intent == ("analytics")

    assert "analytics" in goal.required_capabilities
...
def test_goal_builder_uses_conclusion_as_fallback():
    ...
    assert goal.objective == ("Explain document")

    assert goal.intent == ("document")

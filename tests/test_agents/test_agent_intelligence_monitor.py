...
from src.agents.observability.agent_intelligence_monitor import AgentIntelligenceMonitor
...
from src.agents.evaluation.router_performance_analyzer import RouterPerformanceAnalyzer
...
def test_monitor_generates_report():
    ...
    decision_store.add(create_trace())

    decision_analytics = DecisionAnalytics(decision_store)

    routing_history = RoutingHistory()

    routing_history.add(create_routing())

    routing_metrics = RoutingMetrics(routing_history)

    performance = RouterPerformanceAnalyzer(routing_history)

    monitor = AgentIntelligenceMonitor(decision_analytics, routing_metrics, performance)
...
def test_monitor_contains_routing_metrics():
    ...
    monitor = AgentIntelligenceMonitor(
        DecisionAnalytics(decision_store),
        RoutingMetrics(RoutingHistory()),
        RouterPerformanceAnalyzer(RoutingHistory()),
    )

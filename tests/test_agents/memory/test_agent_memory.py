...
def test_agent_memory_store():
    ...
    entry = memory.store(key="question_1", value="customer analysis")

    assert entry.key == "question_1"

    assert memory.exists("question_1")
...
def test_agent_memory_get():
    ...
    memory.store(key="test", value="information")
...
def test_agent_memory_list():
    ...
    memory.store(key="one", value=1)

    memory.store(key="two", value=2)
...
def test_agent_memory_search():
    ...
    memory.store(key="analysis", value="customer analytics report")

    memory.store(key="other", value="system log")

    result = memory.search("analytics")
...
def test_agent_memory_remove():
    ...
    memory.store(key="remove", value="data")

    memory.remove("remove")

    assert not memory.exists("remove")

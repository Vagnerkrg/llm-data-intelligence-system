...
def test_autonomy_memory_stores_learning_signal():
    ...
    entry = memory.store_learning(signal)

    assert entry.key == "learning-001"

    assert entry.memory_type == "autonomy_learning"
...
def test_autonomy_memory_retrieves_learning():
    ...
    memory.store_learning(signal)

    result = memory.get("learning-002")

    assert result is not None

    assert result.value["pattern"] == "Failure recovery"

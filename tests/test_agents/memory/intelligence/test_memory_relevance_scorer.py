...
def test_should_evaluate_memory_relevance():
    ...
    memory = MemoryEntry(
        memory_id="001",
        content=("User prefers technical explanations with detailed architecture."),
        memory_type=MemoryType.LONG_TERM,
        metadata={"category": "preference"},
    )

    result = scorer.evaluate(memory)

    assert result.memory_id == "001"

    assert result.score > 0

    assert "cognitive_value" in result.factors

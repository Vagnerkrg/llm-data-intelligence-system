from src.agents.memory.utils.memory_mapper import MemoryMapper
...
def test_should_map_memory_to_dict():
    ...
    memory = MemoryEntry(
        memory_id="001", content="mapper test", memory_type=MemoryType.LONG_TERM
    )

    result = MemoryMapper.to_dict(memory)

...
class FakeMultiToolExecutor:
    ...
    def execute(self, tool, question):
        ...
        self.executed_tools.append(tool.name)
        ...
        return type(
            ...
            (),
            {
                "success": True,
                "tool": tool.name,
                "data": {"tool": tool.name, "question": question},
            },
        )()
...
def test_agent_controller_executes_multiple_tools():
    ...
    response = controller.run("Analise produtos e categorias")

    assert response["status"] == "success"

    assert len(executor.executed_tools) >= 1

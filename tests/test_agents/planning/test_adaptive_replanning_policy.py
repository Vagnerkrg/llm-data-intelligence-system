...
def test_policy_keeps_execution_when_everything_is_normal():
    ...
    decision = policy.evaluate(
        ...
        feedback={"requires_adjustment": False},
...
def test_policy_replans_after_failure():
    ...
    decision = policy.evaluate(
        execution_state={"status": "running"}, feedback={}, evaluation={"failed": True}
    )
...
def test_policy_replans_when_feedback_requests_adjustment():
    ...
    decision = policy.evaluate(
        execution_state={},
        feedback={"requires_adjustment": True},
        evaluation={"failed": False},
    )
...
def test_policy_replans_when_execution_is_blocked():
    ...
    decision = policy.evaluate(
        execution_state={"blocked": True}, feedback={}, evaluation={}
    )

...
def test_agent_runtime_stores_reasoning_result():
    ...
    runtime = AgentRuntime(reasoning_engine=MockReasoningEngine())

    context = runtime.prepare("Analyze customer sales")

    assert context.reasoning_result is not None

    assert context.reasoning_result.reasoning == "Test reasoning"

    assert context.reasoning_result.confidence == 0.95

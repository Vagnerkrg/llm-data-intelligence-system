...
class DynamicPlannerRuntimeController(AgentController):
    ...
    def create_plan(self, question: str):
        ...
        return self.dynamic_planner.create_plan(question)
...
def test_agent_runtime_dynamic_planner_integration():
    ...
    runtime = AgentRuntime(controller=controller)

    context = runtime.prepare("Analise os dados de vendas e encontre padrões")
...
def test_agent_runtime_dynamic_planner_generates_expected_plan():
    ...
    context = runtime.prepare("Explique o conteúdo deste documento")
...
def test_agent_runtime_dynamic_planner_supports_multiple_intents():
    ...
    analytics_context = runtime.prepare("Faça uma análise estatística dos dados")

    document_context = runtime.prepare("Resuma este documento PDF")

    assert analytics_context.plan is not None

    assert document_context.plan is not None

    assert len(analytics_context.plan.steps) == len(document_context.plan.steps)

...
def test_dynamic_planner_creates_goal():
    ...
    plan = planner.create_plan("Analyze customer sales data")

    assert plan.goal is not None

    assert plan.goal.objective == "Analyze customer sales data"
...
def test_goal_contains_intent():
    ...
    plan = planner.create_plan("Explique este documento")

    assert plan.goal.intent == "document"
...
def test_goal_driven_plan_keeps_execution_flow():
    ...
    plan = planner.create_plan("Analise os dados")

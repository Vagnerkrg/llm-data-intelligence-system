from src.agents.observability.execution_trace import ExecutionTrace
...
def test_execution_trace_add_event():
    ...
    trace.add_event(event_type="execution_started", payload={"status": "running"})

    assert trace.count_events() == 1

    assert trace.events[0]["type"] == "execution_started"


def test_execution_trace_summary():

    trace = ExecutionTrace(execution_id="exec_001", metadata={"agent": "planner"})

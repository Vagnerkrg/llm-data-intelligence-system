...
def test_router_selects_analytics_tool():
    ...
    result = router.route("Quantos produtos existem?")

    assert result.tool == "analytics"

    assert result.confidence > 0

    assert "Analytical" in result.reason
...
def test_router_selects_analytics_for_customer_query():
    ...
    result = router.route("Quantos clientes temos?")
...
def test_router_returns_no_tool_for_unknown_query():
    ...
    result = router.route("Explique uma teoria qualquer")

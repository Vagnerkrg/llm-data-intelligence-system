...
def test_step_executor_route_request():
    ...
    executor = StepExecutor(controller=FakeController())

    step = PlanStep(step_id=1, action="route_request", description="Route request")

    result = executor.execute(step, "test question")
...
def test_step_executor_execute_tool():
    ...
    result = executor.execute(step, "customer analysis")
...
def test_step_executor_generate_response():
    ...
    result = executor.execute(step, "question")
...
def test_step_executor_unknown_action():
    ...
    step = PlanStep(step_id=99, action="unknown", description="Unknown action")

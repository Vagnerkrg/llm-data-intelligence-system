...
class FakeExecutor:
    def execute(self, step, question):
...
def create_context():
    ...
    context = ExecutionContext(question="test")

    plan = ExecutionPlan(objective="test")

    plan.add_step(PlanStep(step_id=1, action="step_one", description="First step"))

    plan.add_step(PlanStep(step_id=2, action="step_two", description="Second step"))
...
def test_execution_engine_executes_plan():
    ...
    engine = ExecutionEngine(step_executor=FakeExecutor())
...
def test_execution_engine_handles_failure():
    ...
    engine = ExecutionEngine(step_executor=FailingExecutor())

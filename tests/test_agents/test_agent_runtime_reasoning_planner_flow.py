...
def test_agent_runtime_reasoning_influences_planning():
    ...
    context = runtime.prepare("Analyze product sales performance")

    assert context.reasoning is not None

    assert isinstance(context.reasoning, ReasoningResult)

    assert context.plan is not None

    assert len(context.plan.steps) == 3

    assert context.plan.objective == "Analyze product sales performance"

...
def test_alternative_evaluator_evaluates_alternatives():
    ...
    context = DecisionContext(goal="Optimize workflow", available_tools=["python"])
    ...
    evaluations = evaluator.evaluate(alternatives, context)

    assert isinstance(evaluations, list)
    assert len(evaluations) > 0

    assert isinstance(evaluations[0], AlternativeEvaluation)

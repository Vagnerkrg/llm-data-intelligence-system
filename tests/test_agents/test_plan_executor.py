...
def create_test_context():
    ...
    plan = ExecutionPlan(objective="Test execution plan")

    plan.add_step(
        PlanStep(step_id=1, action="route_request", description="Route request")
    )

    plan.add_step(
        PlanStep(step_id=2, action="generate_response", description="Generate response")
    )

    context = ExecutionContext(question="Test question")
